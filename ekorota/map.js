...
window.createBusIconWithPlate = function(plateNumber, bearing, typeOverride = null) {
  ...
  const svgSize = 36; // Tüm araçlar standart 36px
  ...
  const iconHtml = `
    <div style="position:relative;">
      <svg width="${svgSize}" height="${svgSize}" viewBox="0 0 72 72" style="transform:rotate(${bearing-90}deg); filter: drop-shadow(0 3px 6px rgba(0,0,0,0.35)); overflow:visible;">
        ${vehicleSvg}
      </svg>
      <div style="position:absolute; top:-25px; left:50%; transform:translateX(-50%); 
                  background: linear-gradient(135deg, ${mainColor} 0%, ${shadowColor} 100%); 
                  color:white; font-weight:900; padding:2px 7px; 
                  border-radius:6px; border:2px solid white; 
                  box-shadow:0 3px 8px rgba(0,0,0,0.3); 
                  font-size:10.5px; text-align:center; white-space:nowrap; 
                  min-width:44px; font-family: 'Inter', sans-serif;
